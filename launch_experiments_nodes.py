...
Arguments = collections.namedtuple('Arguments', [
    'deployment_size', 'closest_k'])
Arguments.__new__.__defaults__ = (_BASE_DEPLOYMENT_SIZE, _BASE_CLOSEST_K)
...
def run_problem(filename, arguments):
  graph = graph_map.GraphMap(_NUM_NODES, 1, closest_k=arguments.closest_k)
...
def run(final_filename):
  ...
  for closest_k in _CLOSEST_K:
    args.add(Arguments(closest_k=closest_k))
  all_args = []
